...
class Solver(object):
    ...
    def load_model(self):
        ...
        self.model = vgg(model_name="vgg2", num_classes=2, init_weights=True)

        # Adagrad
        self.optimizer = optim.Adagrad(self.model.parameters(), lr=self.lr)
        
        self.scheduler = optim.lr_scheduler.MultiStepLR(self.optimizer, milestones=[75, 150], gamma=0.5)
        self.criterion = nn.CrossEntropyLoss().to(self.device) #criterion定义为交叉熵损失
...
if __name__ == '__main__':
    ...
    solver.test_with_select_model("model_1000.pth",c=0)  
    
    solver.test_with_select_model("model_1000.pth",c=1) 
